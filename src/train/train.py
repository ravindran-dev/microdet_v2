    ...
    if isinstance(preds, (list, tuple)):
        ...
        if all(isinstance(p, torch.Tensor) for p in preds):
            ...
            msg = (
                ...
                "     e.g. in a Python REPL: `from src.model.model_wrapper import MicroDet; m=MicroDet(cfg['model']); print(m)`\n"
                "  2) print the model cfg your wrapper received: `print(cfg['model'])` and ensure head is configured and parsed\n"
                "  3) inspect src/model/model_wrapper.py parsing code and ensure it constructs/instantiates head modules\n\n"
